...
struct Registry {
   ...
   private:
    ...
    ColoredSet grey_;   /// may or may not have references to objects in the
    ...
    // Handles that must survive GC regardless of reachability from roots_.
    // Treated as extra roots at the start of every TriColor() pass.
    Handles pinned_;
   ...
   public:
    ...
    bool Pin(Handle);
    bool Unpin(Handle);
    bool IsPinned(Handle) const;
    bool IsPinned(const Object &Q) const { return IsPinned(Q.GetHandle()); }
