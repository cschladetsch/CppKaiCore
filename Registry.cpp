...
void Registry::DestroyObject(Handle handle, bool force) {
    ...
    try {
        ...
        if (const auto pin = pinned_.find(handle); pin != pinned_.end())
            pinned_.erase(pin);
    ...
    if (!succeeded) {
        ...
        if (auto const found = instances_.find(handle);
            ...
            KAI_TRACE_ERROR() << "Force removing object with handle " << handle
                              << " from instances";
            // This erase does NOT prevent a memory leak - it only removes the
            // dangling bookkeeping entry from instances_. If
            // klass->Delete(base) threw partway through, the StorageBase (and
            // whatever it owned) is never actually freed here: we have no
            // way to know how much of a type-specific, possibly-templated
            // Delete() ran before it threw, so retrying or manually freeing
            // risks a double-free instead. This trades a leak for avoiding
            // that risk. See failed_deletions_ below for what's leaked.
            instances_.erase(found);
...
StorageBase *Registry::GetStorageBase(Handle handle) const {
    // Note: this used to wrap the lookup below in try/catch(...) plus a
    // "volatile void* test" null-check on the already-non-null pointer we
    // just read out of the map. Neither does anything useful - std::map::find
    // on a valid map doesn't throw, and a volatile round-trip cannot detect a
    // dangling/corrupted pointer (that's UB territory, not something a C++
    // exception handler can catch). Removed; this is a plain lookup.
...
void Registry::Delete(Handle handle) {
    // Tri-color GC owns all destruction via DestroyObject()/ReleaseWhite().
    // The pre-tricolor mark/sweep implementation of this function was dead
    // code (KAI_USE_TRICOLOR is unconditionally defined above) and did not
    // compile on its own - it referenced a stale `retained_` member instead
    // of `retainedObjects_`. Removed rather than repaired: if a non-tricolor
    // collector is ever reintroduced, write this against the current
    // Registry member set from scratch instead of reviving the old branch.
...
Pointer<ClassBase const *> Registry::AddClass(Type::Number N,
    ...
    // Deliberately not fixed as part of this pass: this always returns an
    // empty/invalid Pointer<ClassBase const*> even on success. Every current
    // caller (Registry::AddClass<T>() via NewClass<T>(), and the direct call
    // sites in Console.cpp) discards the return value, so this is dormant
    // rather than actively wrong today. A correct fix needs classes to
    // become real Registry-tracked Objects (classes_ is a raw
    // std::vector<ClassBase const*>, entirely separate from instances_/the
    // GC-managed object graph), which is a small design decision someone
    // should make deliberately rather than have it default in here.
    return Pointer<ClassBase const *>();
}

Registry::Percentage Registry::CalcMemoryUsage() const {
    KAI_NOT_IMPLEMENTED();
}
Registry::Percentage Registry::CalcMemoryFragmentationPercentage() const {
    KAI_NOT_IMPLEMENTED();
}
void Registry::DefragmentMemory() { KAI_NOT_IMPLEMENTED(); }

bool Registry::Pin(Handle handle) {
    if (handle == Handle(0)) return false;
    if (instances_.find(handle) == instances_.end()) return false;

    pinned_.insert(handle);
    return true;
}

bool Registry::Unpin(Handle handle) {
    const auto found = pinned_.find(handle);
    if (found == pinned_.end()) return false;

    pinned_.erase(found);
    return true;
}

bool Registry::IsPinned(Handle handle) const {
    return pinned_.find(handle) != pinned_.end();
}
...
void MarkObjectAndChildren(StorageBase &storage, bool marked) {
    // The previous guard here only skipped a node referencing itself
    // directly ("HACK to sorta/kinda avoid cycles"), so any longer cycle
    // (A -> B -> A) still recursed forever. Fixed-point check instead: if
    // this node is already in the target mark state, it (and everything
    // reachable from it) was already visited on this pass, so stop. Marking
    // happens before recursing into children, so a cycle looping back to
    // this node mid-traversal will already see the updated state.
    if (storage.IsMarked() == marked) return;
...
void Registry::TriColor() {
    ...
    // Pinned handles are treated as extra roots for this pass: promoting
    // them to grey_ here means TriColor's own reachability walk (below)
    // keeps them - and anything they reference - out of white_, so
    // ReleaseWhite() never collects them. Stale handles (pinned but since
    // destroyed some other way) are skipped rather than asserted on, since
    // Unpin isn't always called symmetrically with object destruction.
    for (const Handle handle : pinned_) {
        StorageBase *base = GetStorageBase(handle);
        if (base != nullptr) SetColor(*base, ObjectColor::Grey);
    }
...
bool Registry::IsValid() const {
    // Note: this previously wrapped everything in try/catch(...) and used a
    // "volatile void*" round-trip to "test" the allocator pointer - neither
    // catches a dangling/corrupted `this` or allocator_ (that's UB, not a
    // throwing condition in C++; a real crash here is a segfault, not an
    // exception try/catch can intercept). It also required at least one
    // *registered class* to consider the registry valid, which meant a
    // brand-new Registry - allocator set, classes_ freshly sized and empty -
    // reported IsValid() == false until the first AddClass() call. That's
    // backwards: emptiness of classes_ content isn't an invariant violation,
    // only its size (set once in Construct()) is.
    return allocator_ != nullptr && classes_.size() > 0;
}
