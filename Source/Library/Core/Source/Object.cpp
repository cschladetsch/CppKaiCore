...
Object &Object::operator=(Object const &Q) {
    // Reject obviously invalid source pointer.
    if (reinterpret_cast<uintptr_t>(&Q) < 0x1000) {
        return *this;
    }
...
Type::Number Object::GetTypeNumber() const {
    // Check class_base directly rather than calling Valid(), because Valid()
...
bool Object::Valid() const {
    // Reject obviously invalid 'this' pointer (near-null addresses are never
    // valid user-space). Protects against use of Objects stored at corrupt
    // or zeroed memory locations.
...
StorageBase &GetStorageBase_(Object const &Q) {
    ...
    if (registry == nullptr) KAI_THROW_0(NullObject);

    StorageBase *base = registry->GetStorageBase(Q.GetHandle());
    if (base == nullptr) KAI_THROW_0(NullObject);

    return *base;
}

StorageBase &GetStorageBase(Object const &Q) {
    if (!Q.Exists()) KAI_THROW_0(NullObject);
...
BinaryStream &operator>>(BinaryStream &stream, Object &extracted) {
    ...
    int type_number = 0;
    stream >> type_number;
    std::cerr << "[THAW] type_number=" << type_number
              << " registry=" << (stream.GetRegistry() != nullptr)
              << " canread=" << stream.CanRead(1) << "\n";
    if (type_number == 0) {
        extracted = Object();
        return stream;
    }
