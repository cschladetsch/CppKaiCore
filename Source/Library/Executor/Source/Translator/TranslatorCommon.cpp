...
void TranslatorCommon::Append(Object const &ob) {
    try {
        ...
        // PATCH: For binary operations with literal values, we directly evaluate the result
        // at translation time rather than creating a continuation that will be evaluated later
        
        // For literal integers with binary operations, perform direct evaluation
        if (Top()->GetCode()->Size() >= 2 && ob.IsType<Operation>()) {
            Operation::Type opType = ConstDeref<Operation>(ob).GetTypeNumber();
            
            // Only handle binary operations that have exactly 2 operands on the stack
            if ((opType == Operation::Plus || opType == Operation::Minus || 
                ...
                if (val1.Valid() && val2.Valid()) {
                    ...
                    if (val1.IsType<int>() && val2.IsType<int>() && 
                        ...
                        KAI_TRACE() << "TranslatorCommon::Append: Directly evaluated " << num1 
                                 << " " << Operation::ToString(opType) << " " << num2 
                                 << " = " << result;
                        return;
                    }
                    // Handle boolean operations
                        ...
                        code->RemoveAt(code->Size() - 1);  // Remove val2
                        code->RemoveAt(code->Size() - 1);  // Remove val1
                        code->Append(reg_->New<bool>(result));  // Add the result
                        
                        KAI_TRACE() << "TranslatorCommon::Append: Directly evaluated " 
                                 << (b1 ? "true" : "false") << " " 
                    ...
                    // Handle string concatenation
                        ...
                        code->RemoveAt(code->Size() - 1);  // Remove val2
                        code->RemoveAt(code->Size() - 1);  // Remove val1
                        code->Append(reg_->New<String>(result));  // Add the result
                        
                        KAI_TRACE() << "TranslatorCommon::Append: Directly evaluated string concatenation: " 
                                 << s1 << " + " << s2 << " = " << result;
                        return;
                    }
                }
