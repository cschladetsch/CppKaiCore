...
public:
    ...
    typedef std::unordered_map<Handle, std::weak_ptr<StorageBase>> WeakInstances;
...
public:
    ...
    void AddClass(std::shared_ptr<const ClassBase> klass) {
        ...
        if (type_num >= classes_.size())
            classes_.resize(type_num + 1);
            
        if (classes_[type_num])
