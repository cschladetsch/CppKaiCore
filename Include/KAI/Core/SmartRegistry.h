...
public:
    ...
    using SharedInstances = std::unordered_map<Handle, SharedStoragePtr, HashHandle>;
...
public:
    explicit SmartRegistry(std::shared_ptr<Memory::IAllocator> alloc = nullptr)
        : allocator_(alloc ? alloc : std::make_shared<Memory::StandardAllocator>()) {}
    ...
    size_t GetUniqueCount() const {
        return smart_instances_.size();
    }
    
    /// Get count of shared objects
    size_t GetSharedCount() const {
        return shared_instances_.size();
    }
...
public:
    ...
    RegistryObjectGuard(RegistryObjectGuard&& other) noexcept
        : registry_(other.registry_), handle_(other.handle_), released_(other.released_) {
