...
/// Modern smart pointer wrapper that can interoperate with KAI's Pointer<T> system
template<typename T>
...
public:
    ...
    explicit ModernPointer(std::shared_ptr<T> ptr) : modern_ptr_(std::move(ptr)) {}
    ...
    std::optional<Pointer<T>> GetKaiPointer() const {
        return kai_pointer_;
    }
    ...
    bool unique() const noexcept {
        return modern_ptr_ && modern_ptr_.unique();
    }
...
public:
    ...
    KaiObjectGuard(KaiObjectGuard&& other) noexcept
        : registry_(other.registry_), object_(std::move(other.object_)), 
    ...
    Storage<T>* operator->() { return static_cast<Storage<T>*>(object_.GetBasePtr()); }
    const Storage<T>* operator->() const { return static_cast<const Storage<T>*>(object_.GetBasePtr()); }
};
