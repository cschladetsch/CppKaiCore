...
public:
    using CreatorFunction = std::function<std::shared_ptr<TranslatorCommon>(Registry&)>;
    ...
    std::shared_ptr<TranslatorCommon> CreateTranslator(Language lang, Registry& reg) {
        ...
        struct TranslatorRegistrar_##translatorClass { \
            TranslatorRegistrar_##translatorClass() { \
                TranslatorFactory::Instance().RegisterTranslator( \
                    lang, \
                    [](Registry& reg) -> std::shared_ptr<TranslatorCommon> { \
